...
def decoding_step(
    ...
    if preds_output_file:
        out_file.close()
        logging.info(
            f"Stored the predictions of {cfg.decoding_strategy} decoding at '{preds_output_file}'."
        )
...
def main(cfg: EvalBeamSearchNGramConfig):
    ...
    if cfg.kenlm_model_file:
        ...
        if cfg.decoding_strategy != "maes":
            raise ValueError(
                f"Decoding with kenlm model is supported only for maes decoding algorithm."
            )
    ...
    if cfg.decoding_strategy != "maes":
        cfg.maes_prefix_alpha, cfg.maes_expansion_gamma, cfg.hat_ilm_weight = (
            [0],
            [0],
            [0],
        )
        ...
        with autocast():
            with torch.no_grad():
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    with open(
                        os.path.join(tmpdir, "manifest.json"), "w", encoding="utf-8"
                    ) as fp:
                    ...
                    for test_batch in tqdm(
                        temporary_datalayer, desc="Transcribing", disable=True
                    ):
    ...
    if cfg.decoding_strategy in ["beam", "tsd", "alsd", "maes"]:
        if cfg.beam_width is None or cfg.beam_alpha is None:
            raise ValueError(
                "beam_width and beam_alpha are needed to perform beam search decoding."
            )
        params = {
            ...
            if cfg.preds_output_folder:
                ...
                if cfg.decoding_strategy == "maes":
                    ...
                    if cfg.kenlm_model_file:
                        ...
                        if cfg.hat_subtract_ilm:
                            results_file = (
                                f"{results_file}_hat_ilmw{hp['hat_ilm_weight']}"
                            )
                preds_output_file = os.path.join(
                    cfg.preds_output_folder, f"{results_file}.tsv"
                )
            else:
